...
export default function Register() {
  ...
  const handleCredentialResponse = async (response) => {
    ...
    try {
      ...
      toast.loading('Creating and saving real Google account in MongoDB...', { id: 'googleAuth' });
      ...
      if (res.success) {
        toast.success(`Registered and logged in successfully! Welcome, ${res.user.name}.`, { id: 'googleAuth' });
        router.push('/');
      }
    } catch (err) {
      console.error('Google Sign-in Error:', err);
      toast.error('Google registration failed. Please try again.', { id: 'googleAuth' });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Auto-login instantly using the newly created credentials
        const loginRes = await login(formData.email, formData.password);
        if (loginRes.success) {
          toast.success(`Welcome to DriveFleet, ${formData.name}!`);
          router.push('/');
        }
      }
    } catch (err) {
    ...
    <div className="flex-1 flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-background via-background to-accent/5 relative overflow-hidden">
      ...
      >
        ...
        <form className="space-y-4 mt-6" onSubmit={handleSubmit}>
          <div className="space-y-3.5">
            ...
            <div className="space-y-1.5">
              <label htmlFor="name" className="block text-xs font-extrabold uppercase tracking-wider text-muted/90">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className="block w-full pl-11 pr-4 py-2.5 rounded-xl border border-card-border bg-card/85 text-sm focus:border-accent focus:ring-4 focus:ring-accent/15 outline-none transition-all duration-300 text-foreground font-medium shadow-sm hover:border-accent/40"
                />
              </div>
            </div>
            ...
            {/* Photo URL Input */}
            <div className="space-y-1.5">
              <label htmlFor="photoURL" className="block text-xs font-extrabold uppercase tracking-wider text-muted/90">
                Profile Photo URL
              </label>
              <div className="relative group">
                <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-muted group-focus-within:text-accent transition-colors">
            ...
            <div className="space-y-1.5">
              ...
              <div className="mt-2.5 p-3 rounded-xl border border-card-border/80 bg-card-border/10 space-y-2">
                <p className="text-[10px] font-extrabold tracking-wider uppercase text-muted flex items-center">
                  <ShieldAlert size={12} className="mr-1.5 text-accent animate-pulse" /> Security Checklist
                </p>
                <div className="grid grid-cols-3 gap-2">
                  <span className={`flex items-center text-[10px] font-bold transition-colors ${passwordCriteria.hasUpper ? 'text-green-500' : 'text-muted'}`}>
                    {passwordCriteria.hasUpper ? <Check size={12} className="mr-1 shrink-0" /> : <X size={12} className="mr-1 shrink-0" />}
                    Uppercase
                  </span>
                  <span className={`flex items-center text-[10px] font-bold transition-colors ${passwordCriteria.hasLower ? 'text-green-500' : 'text-muted'}`}>
                    {passwordCriteria.hasLower ? <Check size={12} className="mr-1 shrink-0" /> : <X size={12} className="mr-1 shrink-0" />}
                    Lowercase
                  </span>
                  <span className={`flex items-center text-[10px] font-bold transition-colors ${passwordCriteria.isMinLength ? 'text-green-500' : 'text-muted'}`}>
                    {passwordCriteria.isMinLength ? <Check size={12} className="mr-1 shrink-0" /> : <X size={12} className="mr-1 shrink-0" />}
                    6+ Chars
                  </span>
                </div>
              </div>
          ...
          <div className="pt-2">
            <button
              ...
              className={`w-full py-3 px-4 rounded-xl text-white font-extrabold text-sm shadow-md transition-all duration-200 active:scale-95 flex justify-center items-center ${
                isPasswordValid
                  ? 'bg-gradient-to-r from-accent to-blue-500 hover:from-accent-hover hover:to-blue-600 shadow-accent/15 cursor-pointer hover:-translate-y-0.5'
        ...
        <p className="mt-6 text-center text-xs sm:text-sm text-muted font-medium">
          ...
          <Link href="/login" className="font-extrabold text-accent hover:text-accent-hover transition-colors underline decoration-accent/35">
            Login here
          </Link>
        </p>
      </motion.div>
